...
class MySQLWalletDAO(WalletDAO):
    ...
    def get_reserved_funds(self, client_id: UUID) -> Decimal:
        reserved_funds = ReservedFunds.objects.filter(client_id=client_id).aggregate(total=Sum("amount"))["total"]
    ...
    def reserve_funds(self, client_id: UUID, amount: Decimal, order_id: UUID) -> WalletDTO:
        try:
            with transaction.atomic():
                ReservedFunds.objects.create(client_id=client_id, order_id=order_id, amount=amount)

                return WalletDTO(success=True, code=201)

        except IntegrityError:
            logger.error(f"IntegrityError: client {client_id} already has a reservation for order {order_id},", exc_info=True)
